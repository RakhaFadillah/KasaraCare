...
import { LayoutDashboard, Users, Stethoscope, CalendarClock, LogOut, Layers } from "lucide-react";
...
const menuGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Dokter & Terapis", url: "/admin/doctors", icon: Stethoscope },
    ],
  },
  {
    label: "Layanan & Operasional",
    items: [
      { title: "Jenis Treatment", url: "/admin/services", icon: Layers },
      { title: "Reservasi", url: "/admin/registrations", icon: CalendarClock },
    ],
  },
];
...
export default function AdminSidebar() {
  ...
  return (
    <Sidebar
      collapsible="icon"
      className="border-r border-slate-200 dark:border-slate-800 dark:bg-[#10121b] transition-colors duration-300"
    >
      <SidebarHeader className="border-b border-transparent pt-4 pb-2">
        <div className="flex items-center justify-center w-full px-3 group-data-[collapsible=icon]:px-0">
          {/* Logo yang juga berfungsi sebagai tombol toggle sidebar */}
          <button
            onClick={toggleSidebar}
            className="flex items-center gap-3 w-full group cursor-pointer focus:outline-none"
            title="Sembunyikan/Tampilkan Sidebar"
          >
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl overflow-hidden shadow-lg shadow-blue-600/20 bg-blue-500 group-hover:ring-2 ring-blue-400 transition-all">
            ...
            <div className="flex flex-col text-left truncate group-data-[collapsible=icon]:hidden">
              <h2 className="font-display text-lg font-black tracking-tight text-slate-900 dark:text-white transition-colors group-hover:text-blue-600 dark:group-hover:text-blue-400">
                KasaraCare
              </h2>
            </div>
          </button>
                    ...
                    <SidebarMenuItem key={item.url}>
                      ...
                      >
                        <Link to={item.url} className="flex items-center gap-3 py-2 px-1">
                          <item.icon
                            className={`h-4.5 w-4.5 shrink-0 ${active ? "text-blue-600 dark:text-blue-400" : ""}`}
                          />
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="flex flex-col overflow-hidden">
              ...
              <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 truncate transition-colors">
                {user?.email || "admin@kasaracare.com"}
              </span>
            </div>
          </div>
