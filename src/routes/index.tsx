...
import { Users, ShieldCheck, ArrowRight, BedDouble, ClipboardPlus, PieChart } from "lucide-react";
...
function Landing() {
  ...
  useEffect(() => {
    if (session) {
      // PERBAIKAN: Arahkan langsung ke dashboard admin
      navigate({ to: "/admin" });
    }
  }, [session, navigate]);
    ...
    >
      <header className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-bold text-slate-900 tracking-tight">ChopperCare</div>
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <h1 className="text-4xl font-extrabold leading-[1.15] md:text-[54px] text-slate-900 tracking-tight">
              Pusat Kendali <br/>
              <span style={{ color: '#00a2ed' }}>Operasional RS</span> <br/>
              dalam Genggaman
            </h1>
            
            <p className="mt-6 max-w-lg text-lg text-slate-600 leading-relaxed font-medium">
              Kelola jadwal operasi, manajemen dokter, kapasitas kamar, dan data pasien dalam satu platform yang terintegrasi secara cerdas.
            </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              { icon: Users, title: "Manajemen Medis", desc: "Kelola rekam medis pasien dan jadwal praktik dokter harian." },
              { icon: ClipboardPlus, title: "Jadwal Operasi", desc: "Atur tindakan operasi akurat & pantau status pelaksanaannya." },
              { icon: BedDouble, title: "Fasilitas Kamar", desc: "Pantau kapasitas ranjang & status kelas kamar secara real-time." },
              { icon: PieChart, title: "Analitik Interaktif", desc: "Insight operasional dari grafik dinamis kunjungan & tren hunian." },
            ].map((f) => (
              <div
                key={f.title}
              ...
              >
                ...
                <div className="mt-2 text-sm text-slate-500 leading-relaxed font-medium">{f.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer className="border-t border-slate-200/60 py-8 text-center text-xs text-slate-500 font-semibold tracking-wide">
        © {new Date().getFullYear()} ChopperCare. Seluruh hak cipta dilindungi.
      </footer>
    </div>
  );
}